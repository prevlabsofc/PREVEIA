/**
 * Fixture mínima SM rural para teste de PDF (Ana Lúcia Ferreira).
 * Uso: importar no harness de teste ou colar no gerador.
 */
import {
  FUND_ADI_2110_2111,
  FUND_TEMA_533_STJ,
} from '@/lib/peticoes/fundamentos'
import { valorCausaSalarioMaternidade } from '@/lib/salario-minimo'

const _v = valorCausaSalarioMaternidade('10/01/2025')

export const FIXTURE_SM_ANA_LUCIA = `<<<SM_RURAL_V2>>>
<<<META>>>
tipo_acao: SALÁRIO MATERNIDADE - SEGURADO ESPECIAL
juizo_digital: true
prioridade_idoso: false
prioridade_deficiente: false
prioridade_menor: false
<<<END_META>>>

<<<ENDERECO>>>
AO JUÍZO FEDERAL DO JUIZADO ESPECIAL FEDERAL DA SUBSEÇÃO JUDICIÁRIA DE SÃO LUÍS/MA
<<<END_ENDERECO>>>

<<<QUALIFICACAO>>>
ANA LÚCIA FERREIRA, brasileira, agricultora, nascida em 15/03/1995, RG 1234567 SSP/MA, CPF 000.000.000-00, residente e domiciliada na Zona Rural, Município de São Luís/MA, por intermédio de seus procuradores, vem, respeitosamente, à presença de Vossa Excelência, propor a presente
<<<END_QUALIFICACAO>>>

<<<TITULO>>>
AÇÃO PREVIDENCIÁRIA DE CONCESSÃO DE SALÁRIO-MATERNIDADE
<<<SUBTITULO>>>
(SEGURADA ESPECIAL – AGRICULTORA)
<<<END_TITULO>>>

<<<EM_FACE>>>
Em face do INSTITUTO NACIONAL DO SEGURO SOCIAL – INSS, autarquia federal, a ser citado na Agência da Previdência Social em São Luís/MA.
<<<END_EM_FACE>>>

<<<I_PRELIMINARES>>>
DA GRATUIDADE DA JUSTIÇA:
A autora declara hipossuficiência nos termos do art. 5º, LXXIV, da CF/88 e da Lei 1.060/50, requerendo os benefícios da justiça gratuita.
<<<END_I>>>

<<<II_QUADRO>>>
| Campo | Valor |
| --- | --- |
| Nome | ANA LÚCIA FERREIRA |
| Idade no Req. Adm. | 29 |
| Pedido | Salário-Maternidade – Segurado Especial |
| Criança | João Ferreira |
| Data de Nascimento | 10/01/2025 |
| Data do Req. Adm. | 20/02/2025 |
| NB | 123.456.789-0 |
| Situação/Decisão INSS | Indeferido |
| Data do Indef. Adm. | 15/03/2025 |
| Motivo INSS | Ausência de qualidade de segurado especial |
| Tempo de trabalho antes do parto | Mais de 12 meses |
| Período de Segurado Especial declarado | 2018 a 2025 |
| Ponto controvertido | Qualidade de segurada especial |
| Benefício anterior | Não consta |
| Período averbado no CNIS | Não consta |
| Vínculo urbano | Não possui |
<<<END_II>>>

<<<III_SINTESE_ANTES>>>
A autora labora em regime de economia familiar na zona rural do município, dedicando-se ao cultivo de mandioca e milho, sem empregados permanentes.

Sustenta a família com o produto da lavoura, em condição típica de segurada especial.
<<<END_III_ANTES>>>

<<<TIMELINE>>>
{"nome":"ANA LÚCIA FERREIRA","atividade":"Agricultora","local":"São Luís/MA","estilo":"none","eventos":[]}
<<<END_TIMELINE>>>

<<<III_SINTESE_DEPOIS>>>
Em 10/01/2025 nasceu o filho da autora. Durante o período gestacional manteve-se no trabalho rural. Requereu administrativamente o salário-maternidade, indeferido pelo INSS sem lastro fático adequado.
<<<END_III_DEPOIS>>>

<<<IV_PROVAS>>>
✓ Certidão de nascimento do filho
✓ Documentos de identificação da autora
✓ Declaração de exercício de atividade rural
✓ Autodeclaração de segurado especial (art. 38-B, §2º, Lei 8.213/91)
✓ Notas fiscais de produtor
✓ Contratos de parceria agrícola
<<<END_IV>>>

<<<IV_FECHO>>>
As provas materiais, conjugadas com a prova testemunhal a ser produzida em audiência, comprovam o exercício de atividade rural em regime de economia familiar e o preenchimento da carência. A declaração de sindicato rural figura como prova complementar.
<<<END_IV_FECHO>>>

<<<V_FUNDAMENTACAO>>>
O salário-maternidade do segurado especial encontra amparo no art. 71 e no art. 39, parágrafo único, da Lei 8.213/91, bem como no art. 7º, XVIII, da CF/88.

${FUND_ADI_2110_2111}

${FUND_TEMA_533_STJ}
<<<END_V>>>

<<<VI_PEDIDOS>>>
i. Que todas as comunicações processuais sejam feitas em nome dos advogados constituídos, nos termos do art. 272, §5º, do CPC
ii. A procedência do pedido, com a concessão do salário-maternidade
iii. A averbação do período no CNIS
iv. A citação da ré e a juntada do processo administrativo NB
v. O pagamento de 120 dias de benefício, com atualização nos termos do Manual de Cálculos da Justiça Federal, com incidência da taxa SELIC a partir de dezembro/2021, conforme art. 3º da EC nº 113/2021
vi. A designação de audiência UNA
vii. A concessão da justiça gratuita
viii. O destaque de honorários contratuais de 30% em favor do escritório
<<<END_VI>>>

<<<FECHAMENTO>>>
Protesta o alegado por todos os meios admitidos em direito, especialmente o depoimento pessoal da parte autora e das testemunhas que comparecerão em audiência, independente de intimação.

Dá-se à causa o valor de ${_v.totalFmt} (${_v.total.toLocaleString('pt-BR')} reais), renunciando-se a eventual excedente da alçada do Juizado Especial Federal, especificamente para fins de fixação da competência.

Termos em que, pede e espera deferimento.

ADVOGADO TESTE
OAB/MA nº 12345
<<<END_FECHAMENTO>>>

<<<PLANILHA>>>
| Campo | Valor |
| --- | --- |
| 1º Mês de benefício | ${_v.mensalFmt} |
| 2º Mês de benefício | ${_v.mensalFmt} |
| 3º Mês de benefício | ${_v.mensalFmt} |
| 4º Mês de benefício | ${_v.mensalFmt} |
| TOTAL | ${_v.totalFmt} |
nota: ${_v.nota}
<<<END_PLANILHA>>>
`
